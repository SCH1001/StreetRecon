...
__device__ __forceinline__ void fwd_n_linear(   //这里需要修改
    F get_grid_val,
	...
	if (result_ptr) {
		...
		if(N_POS_DIMS == 3)
		{
			//B样条拟合，这里强制认为输入数据是3维的
			//权重数组
			COMPUTE_T w1_array[KERNEL_NUM] = {(COMPUTE_T)0.5*pos[0]*pos[0]-pos[0]+(COMPUTE_T)0.5, -pos[0]*pos[0]+pos[0]+(COMPUTE_T)0.5, (COMPUTE_T)0.5*pos[0]*pos[0]};
			COMPUTE_T w2_array[KERNEL_NUM] = {(COMPUTE_T)0.5*pos[1]*pos[1]-pos[1]+(COMPUTE_T)0.5, -pos[1]*pos[1]+pos[1]+(COMPUTE_T)0.5, (COMPUTE_T)0.5*pos[1]*pos[1]};
			COMPUTE_T w3_array[KERNEL_NUM] = {(COMPUTE_T)0.5*pos[2]*pos[2]-pos[2]+(COMPUTE_T)0.5, -pos[2]*pos[2]+pos[2]+(COMPUTE_T)0.5, (COMPUTE_T)0.5*pos[2]*pos[2]};
			//偏移数组
			int offset_array[KERNEL_NUM] = {-1, 0, 1};

			uint32_t pos_grid_local[N_POS_DIMS];
			#pragma unroll 1
			for(uint32_t idx = 0; idx < KERNEL_NUM; idx++)
			{
				pos_grid_local[0] = (uint32_t)((int)pos_grid[0] + offset_array[idx]);
				#pragma unroll
				for(uint32_t idy = 0; idy < KERNEL_NUM; idy++)
				{
					pos_grid_local[1] = (uint32_t)((int)pos_grid[1] + offset_array[idy]);
					#pragma unroll
					for(uint32_t idz = 0; idz < KERNEL_NUM; idz++)
					{
						pos_grid_local[2] = (uint32_t)((int)pos_grid[2] + offset_array[idz]);
						COMPUTE_T weight = w1_array[idx]*w2_array[idy]*w3_array[idz];    //得到权重
	...
	if (grads_ptr) { 
		if(N_POS_DIMS == 3)
		{
			...
			COMPUTE_T w3_array[KERNEL_NUM] = {(COMPUTE_T)0.5*pos[2]*pos[2]-pos[2]+(COMPUTE_T)0.5, -pos[2]*pos[2]+pos[2]+(COMPUTE_T)0.5, (COMPUTE_T)0.5*pos[2]*pos[2]};
			//权重导数数组
			COMPUTE_T w1_grad_array[KERNEL_NUM] = {pos[0]-(COMPUTE_T)1, -(COMPUTE_T)2*pos[0]+(COMPUTE_T)1, pos[0]};
			COMPUTE_T w2_grad_array[KERNEL_NUM] = {pos[1]-(COMPUTE_T)1, -(COMPUTE_T)2*pos[1]+(COMPUTE_T)1, pos[1]};
			COMPUTE_T w3_grad_array[KERNEL_NUM] = {pos[2]-(COMPUTE_T)1, -(COMPUTE_T)2*pos[2]+(COMPUTE_T)1, pos[2]};
			//偏移数组
			int offset_array[KERNEL_NUM] = {-1, 0, 1};

			uint32_t pos_grid_local[N_POS_DIMS];
			#pragma unroll 1
			for (uint32_t grad_dim = 0; grad_dim < N_POS_DIMS; grad_dim++) {   //对每一维求导
				#pragma unroll
				for(uint32_t idx = 0; idx < KERNEL_NUM; idx++)
				{
					pos_grid_local[0] = (uint32_t)((int)pos_grid[0] + offset_array[idx]);
					...
					for(uint32_t idy = 0; idy < KERNEL_NUM; idy++)
					{
						...
						for(uint32_t idz = 0; idz < KERNEL_NUM; idz++)
						{
							...
							if(grad_dim == 0)
								weight *= (w1_grad_array[idx]*w2_array[idy]*w3_array[idz]);
							else if(grad_dim == 1)
								weight *= (w1_array[idx]*w2_grad_array[idy]*w3_array[idz]);
							else if(grad_dim == 2)
								weight *= (w1_array[idx]*w2_array[idy]*w3_grad_array[idz]);

							//根据体素坐标得到对应的特征索引
							auto val = grid_val(pos_grid_local);
							#pragma unroll
							for (uint32_t f = 0; f < N_FEAT; ++f) {
								grads_ptr[f][grad_dim] += weight * (COMPUTE_T)val[f];
							}
						}
					}
				}
...
__device__ __forceinline__ void bwd_input_bwd_grid_n_linear(    //这里需要修改
	F add_grid_grad, 
	const COMPUTE_T scale[N_POS_DIMS],
	const COMPUTE_T pos[N_POS_DIMS],
	const COMPUTE_T pos_derivative[N_POS_DIMS],
...
template <typename INPUT_T, typename PARAM_T, typename COMPUTE_T, uint32_t N_POS_DIMS, uint32_t N_FEAT, typename F>
__device__ __forceinline__ void bwd_input_bwd_input_n_linear(   //这里需要修改
	F calc_dLdx_dim,
	const COMPUTE_T scale[N_POS_DIMS],
...
inline void lod_bwd_impl_dispatched(
	...
	if (need_param_grad) {
		...
		static constexpr uint32_t N_THREADS_LOD = 128;
